# Main figure minimal reproduction from the final 109-study coded dataset
...
pkgs <- c(
  ...
  "forcats", "patchwork", "ggrepel", "scales", "rlang",
  "cowplot", "ragg", "tibble"
)
...
if (!file.exists(input_file)) {
  stop(
    ...
    "\nPlease place this script and 'Supplementary Data File 2_Study-level coding.xlsx' in the same folder."
  )
}
...
if (nrow(dat) != 109) {
  warning("Expected 109 rows, but found: ", nrow(dat))
}
...
dat <- dat %>%
  ...
  dplyr::mutate(
    ...
    `Platform classification (primary)` = dplyr::case_when(
      `Platform classification (primary)` == "Other suction-enabled platform" ~ "Other platforms",
...
save_plot <- function(p, filename_base, width = 8, height = 5, dpi = 600) {
  ggplot2::ggsave(
...
build_stack_rect_df <- function(df, stage_col, cat_col, levels_order, label_col_map,
  ...
  df %>%
    ...
    dplyr::mutate(
      ...
      # integer percentage labels summing to 100 within each stage
      pct_raw   = Share * 100,
      pct_floor = floor(pct_raw),
      pct_frac  = pct_raw - pct_floor,
      pct_rank  = rank(-pct_frac, ties.method = "first"),
      pct_add   = dplyr::if_else(
        pct_rank <= (100 - sum(pct_floor)),
        1, 0
      ),
...
save_plot(fig1_combo, "Figure_1_Combined", width = 12.8, height = 8.85)
...
heat_df <- dat %>%
  ...
  dplyr::mutate(
    ...
    LabelCol = dplyr::if_else(ShareWithinPlatform >= 0.55, "white", TEXT_GREY)
  ) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(
...
p2a <- ggplot2::ggplot(
  ...
  ggplot2::theme(
    ...
    axis.text.y = ggplot2::element_text(size = 9.8, color = TEXT_GREY),
...
label_df <- landmark_df %>%
  ...
  dplyr::mutate(
    nudge_x = dplyr::case_when(
      ...
      Indication_Label == "Local-anesthesia pathway"     & `Publication year` == 2023 ~  0.04,
    ...
    nudge_y = dplyr::case_when(
      ...
      Indication_Label == "Local-anesthesia pathway"     & `Publication year` == 2023 ~ -0.13,
...
size_breaks_fig2 <- size_breaks_fig2[size_breaks_fig2 <= max(landmark_df$BubbleN_plot, na.rm = TRUE)]

if (length(size_breaks_fig2) == 0) {
  size_breaks_fig2 <- pretty(c(0, max(landmark_df$BubbleN_plot, na.rm = TRUE)), n = 4)
...
p2b <- ggplot2::ggplot(
  ...
  ggplot2::geom_point(
    ggplot2::aes(size = BubbleN_plot, fill = `Study design maturity`),
    shape = 21,
    color = "grey28",
    stroke = 0.36,
    alpha = 0.92,
  ...
  ggplot2::theme(
    ...
    panel.grid.major.x = ggplot2::element_blank(),
    panel.grid.minor = ggplot2::element_blank(),
    axis.title.x = ggplot2::element_text(size = 11.0, color = TEXT_GREY),
    axis.text.x = ggplot2::element_text(size = 9.6, color = TEXT_GREY),
    axis.text.y = ggplot2::element_text(size = 9.8, color = TEXT_GREY),
    plot.tag = ggplot2::element_text(size = 12.0, face = "bold", color = TEXT_GREY),
    legend.position = "right",
    legend.box = "vertical",
    legend.title = ggplot2::element_text(size = 10.2, color = TEXT_GREY),
    legend.text = ggplot2::element_text(size = 9.3, color = TEXT_GREY),
...
save_plot(fig2_combo, "Figure_2_Combined", width = 12.8, height = 8.85)
